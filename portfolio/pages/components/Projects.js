...
const Projects = () => {
  ...
  return (
        ...
        <div className="w-auto lg:flex gap-10">
            ...
            <div className=" p-10 rounded-xl my-10 mb-72 flex-1 dark:text-gray-200">
          <h3 className="text-3xl mb-12 text-center font-medium  dark:text-gray-200">All Projects List</h3>
                
                <ul className="list-disc lg:px-60 md:px-48">
                  {data.map((project) => {
                    // let pImg = project.images.static
                    return (
                      <li className=" hover:text-teal-500 duration-200 my-5 cursor-pointer" onMouseOver={e => handleMouse(e.target.innerText)}>{project.name}</li>
                    )
                  })}
                </ul>
            </div>
